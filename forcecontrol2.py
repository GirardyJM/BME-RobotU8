...
def create_transformation_matrix(starting_point):
    ...
    p1 = starting_point + np.array(
        [-point_spacing / 2, 0, 0])  # Creating point p1 by moving half a spacing left from starting point
    p2 = starting_point  # Setting p2 as the starting point
    p3 = starting_point + np.array(
        [point_spacing / 2, 0, 0])  # Creating point p3 by moving half a spacing right from starting point
...
def force_control_6d(arm, r_offset, F_target):
    kp_pos = 0.01   # Proportional gain for translational force control
    kp_ori = 0.05   # Proportional gain for rotational torque control
    threshold = 0.1  # Convergence threshold for all 6 force/torque components
    max_iterations = 150

    for i in range(max_iterations):
        # Read force/torque from the sensor and transform to TCP
        raw_force = np.array(arm.ft_ext_force)
        tcp_force = transform_wrench(raw_force, r_offset)

        # Compute control error: target - actual
        F_error = F_target - tcp_force

        # Exit early if all force components are within threshold
        if np.all(np.abs(F_error) < threshold):
            break

        # Get the current TCP position
        ret = arm.get_position()
        if not isinstance(ret, (tuple, list)) or len(ret) != 2 or ret[0] != 0:
            continue
        pos = ret[1]  # Current pose: [x, y, z, roll, pitch, yaw]

        # Compute incremental pose adjustment based on force error
        delta = np.array([
            kp_pos * F_error[0],
            kp_pos * F_error[1],
            kp_pos * F_error[2],
        ...
        new_pose = [pos[j] + delta[j] for j in range(6)]

        # Check and recover robot state if not ready
        state = arm.get_state()
        if state != 0:
            arm.set_state(0)
            time.sleep(0.5)
        ...
        arm.set_position(*new_pose, wait=True)
        time.sleep(0.2)
# ---------- Transform Sensor Wrench to TCP ----------
...
# ---------- Main Function ----------
def run_force_to_gs_analysis(F_target, ip='192.168.1.197', starting_point=None):
    """
    Run full analysis based on desired 6D wrench:
    - Connect to robot
    - Enable & zero the FT sensor
    - Apply force control to reach F_target
    - Read current TCP position
    - Convert to Grood-Suntay angles
    - Return [IE, FE, VV] + [x, y, z]
    """
    if starting_point is None:
        starting_point = np.array([597, -31.4, 317.7])
    r_offset = [0.009, 0.0, 0.0]  # fixed sensor-to-TCP offset in meters

    # Step 1: Connect to robot with sensor reporting enabled
    ...
    # [DEBUG] 确认传感器是否返回数据
    print("[DEBUG] First sensor reading:", arm.ft_ext_force)

    # Step 2: Apply force control
    force_control_6d(arm, r_offset, F_target)

    # Step 3: Read TCP pose safely
    ret = arm.get_position()
    if not isinstance(ret, (tuple, list)) or len(ret) != 2 or ret[0] != 0:
        arm.ft_sensor_enable(0)
        arm.disconnect()
        raise RuntimeError("Failed to get TCP position from robot.")

    tcp_pose = ret[1]  # [x, y, z, roll, pitch, yaw]
    tcp_xyz = np.array(tcp_pose[:3])
    roll, pitch, yaw = np.radians(tcp_pose[3:6])

    # Step 4: Compute GS angles from transformation
    T_robot2knee, femur_origin = create_transformation_matrix(starting_point)
    R_tcp = euler_to_matrix(roll, pitch, yaw)
    T_knee2tcp = np.eye(4)
    T_knee2tcp[:3, :3] = R_tcp
    T_knee2tcp[:3, 3] = tcp_xyz - femur_origin
    gs_angles = calculate_gs_angles(T_knee2tcp)  # [IE, FE, VV]
    ...
    # Step 6: Output
    print("\n===== Force → GS Estimation Result =====")
    print("Input Force [Fx, Fy, Fz, Mx, My, Mz] (N / Nm):", np.round(F_target, 2))
    print("TCP Position [x, y, z] (mm):", np.round(tcp_xyz, 2))
    print("Estimated GS Angles [IE, FE, VV] (°):", np.round(gs_angles, 2))
    print("=========================================\n")

    return gs_angles, tcp_xyz

# Example input: target wrench (Fx, Fy, Fz, Mx, My, Mz)
F_target = np.array([5.0, 5.0, -3.0, 0.2, -0.1, 0.05])

# Call the analysis function
gs_angles, tcp_xyz = run_force_to_gs_analysis(F_target)

# Optionally, print the result again here
print("→ Final TCP Position [x, y, z] (mm):", np.round(tcp_xyz, 2))
print("→ Estimated GS Angles [IE, FE, VV] (°):", np.round(gs_angles, 2))