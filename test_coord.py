...
starting_point = np.array([500, 1.8, 100]) #initial position to base
...
flexion = 95
rotation = 145
#assumption: angle is from 0-180'''
...
arm.move_gohome()
arm.set_position(x=starting_point[0],y=starting_point[1],z=starting_point[2],speed=80,is_radian=False,wait = True)
#arm.set_pause_time(0.5)
arm.set_position(x=transB[0], y=transB[1], z=transB[2],roll=add,pitch=flexion,yaw=rotation,speed=80,is_radian=False,wait=False)
#arm.set_pause_time(0.5)
#arm.set_tool_position(0,0,0,roll=add,pitch=flexion,yaw=rotation,wait=False)
